...
# Removed LaTeX rendering and custom fonts to use Matplotlib defaults
plt.rcParams.update({
    "font.family": "serif",
    "font.size": 16,
    "axes.titlesize": 16,
...
def plot_history(file_number):
    ...
    # Matplotlib's built-in mathtext handles $C_D$ without a LaTeX installation
    ax.set_ylabel(r"$C_D$ [1]")
    ax.set_xlabel(r"Iteration")
    
    # Replaced LaTeX \textbf with native fontweight argument
    ax.set_title("Drag Coefficient History", fontweight='bold')
    ...
    # Replaced LaTeX \textbf with native fontweight argument
    ax.text(0.98, 0.95, f'Avg $C_D$: {label_cd}', 
            transform=ax.transAxes, ha='right', va='top', bbox=bbox_props, fontweight='bold')
